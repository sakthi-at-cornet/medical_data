...
def test_max_messages_limit():
    ...
    # Should only keep last max_session_messages (default 30)
    assert len(manager.sessions[session_id]) == settings.max_session_messages
    assert manager.sessions[session_id][0].content == f"Message {35 - settings.max_session_messages}"
